...
def auth_from_ctx(ctx: Context | None) -> DitraSoftwareAuth:
    if ctx is None:
...
def apply_default_auth(auth: DitraSoftwareAuth, *, default_api_key: str | None) -> DitraSoftwareAuth:
...
def require_auth(auth: DitraSoftwareAuth) -> None:
    if auth.access_token or auth.api_key:
        return
    raise ToolError(
