...
class DitraSoftwareAuth:
    ...
    def merged(self, other: DitraSoftwareAuth) -> DitraSoftwareAuth:
...
class DitraSoftwareRestError(RuntimeError):
    def __init__(self, message: str, *, status_code: int | None = None):
        super().__init__(message)
        self.status_code = status_code


class DitraSoftwareRestClient:
    def __init__(self, settings: DitraSoftwareSettings):
    ...
    async def request(
        ...
        *,
        ...
        json: Any | None = None,
        auth: DitraSoftwareAuth | None = None,
        extra_headers: Mapping[str, str] | None = None,
        expect_json: bool = True,
    ) -> Any:
        if not self._settings.api_base_url:
            raise DitraSoftwareRestError(
                "DitraSoftware REST backend is not configured: set DITRASOFTWARE_API_BASE_URL"
            )
        ...
        async with httpx.AsyncClient(verify=self._settings.verify_ssl, timeout=timeout) as client:
            try:
                ...
                raise DitraSoftwareRestError(f"REST request failed: {e}") from e
            ...
            if resp.status_code >= 400:
                ...
                raise DitraSoftwareRestError(error_text, status_code=resp.status_code)
