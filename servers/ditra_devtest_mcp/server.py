...
from .rest_client import DitraDevtestAuth, DitraDevtestRestClient
...
def _patch_fastmcp_prefab_synth_domain() -> None:
    ...
    if getattr(prefab_synthesis, "_ditra_devtest_domain_patch", False):
        return
    ...
    prefab_synthesis._ditra_devtest_domain_patch = True
...
def _header_auth(ctx: Context | None) -> DitraDevtestAuth:
...
def _auth_from_ctx(ctx: Context) -> DitraDevtestAuth:
...
def _require_auth(auth: DitraDevtestAuth) -> None:
...
def _apply_default_auth(auth: DitraDevtestAuth, *, default_api_key: str | None) -> DitraDevtestAuth:
    if not default_api_key:
        return auth
    if auth.api_key:
        return auth
    return auth.merged(DitraDevtestAuth(api_key=default_api_key))
...
def _should_advertise_hashed_tool_aliases(ctx: Any) -> bool:
    ...
    mode = _env_mode("DITRA_DEVTEST_HASHED_TOOL_ALIASES", "auto")
    if mode == "always":
        return True
    if mode == "never":
        return False
...
def _should_include_legacy_hashed_aliases(ctx: Any) -> bool:
    ...
    mode = _env_mode("DITRA_DEVTEST_LEGACY_HASHED_TOOL_ALIASES", "auto")
    if mode == "always":
        return True
    if mode == "never":
        return False
...
def create_mcp() -> FastMCP:
    ...
    client = DitraDevtestRestClient(settings)
    ...
    class _StripToolHashMiddleware(Middleware):
        ...
        async def on_list_tools(self, context, call_next):
            ...
            app_names_for_hash = ["DitraDevtest"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="DitraDevtest Template MCP (FastMCP)")
    parser.add_argument(
        "--transport",
        type=str,
        default="http",
