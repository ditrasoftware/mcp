...
def _candidate_env_names(remote: RemoteBackendSettings, key: str) -> list[str]:
    ...
    if remote_name:
        names.append(f"DITRA_DEVTEST_GATEWAY_REMOTE_{remote_name}_{key}")
    if remote_namespace and remote_namespace != remote_name:
        names.append(f"DITRA_DEVTEST_GATEWAY_REMOTE_{remote_namespace}_{key}")

    names.append(f"DITRA_DEVTEST_GATEWAY_REMOTE_{key}")
...
def _ensure_auth_configured(remote: RemoteBackendSettings) -> None:
    ...
    missing_display = ", ".join(missing) if missing else "remote bearer/refresh settings"
    raise GatewayAuthConfigurationError(
        "Remote auth is not configured for this OAuth-protected downstream MCP. "
