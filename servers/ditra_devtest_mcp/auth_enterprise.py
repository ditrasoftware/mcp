...
from .rest_client import DitraDevtestAuth
...
def audit_log(event: AuditEvent, settings: DitraDevtestSettings) -> None:
...
def should_refresh_token(token_info: TokenInfo, settings: DitraDevtestSettings) -> bool:
...
def is_token_revoked(token: str, settings: DitraDevtestSettings) -> bool:
...
def has_scope(token_scopes: list[str] | None, required_scope: str, settings: DitraDevtestSettings) -> bool:
...
def requires_mfa(settings: DitraDevtestSettings, risk_score: float | None = None) -> bool:
...
def extract_user_identity(auth: DitraDevtestAuth, settings: DitraDevtestSettings) -> dict[str, Any]:
