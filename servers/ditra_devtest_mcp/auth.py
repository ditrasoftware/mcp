...
def auth_from_ctx(ctx: Context | None) -> DitraDevtestAuth:
    if ctx is None:
...
def apply_default_auth(auth: DitraDevtestAuth, *, default_api_key: str | None) -> DitraDevtestAuth:
...
def require_auth(auth: DitraDevtestAuth) -> None:
    if auth.access_token or auth.api_key:
        return
    raise ToolError(
