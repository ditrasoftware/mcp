...
class LottomaticapssAuth:
    ...
    def merged(self, other: LottomaticapssAuth) -> LottomaticapssAuth:
...
class LottomaticapssRestError(RuntimeError):
    def __init__(self, message: str, *, status_code: int | None = None):
        super().__init__(message)
        self.status_code = status_code


class LottomaticapssRestClient:
    def __init__(self, settings: LottomaticapssSettings):
    ...
    async def request(
        ...
        *,
        ...
        json: Any | None = None,
        auth: LottomaticapssAuth | None = None,
        extra_headers: Mapping[str, str] | None = None,
        expect_json: bool = True,
    ) -> Any:
        if not self._settings.api_base_url:
            raise LottomaticapssRestError(
                "Lottomaticapss REST backend is not configured: set LOTTOMATICAPSS_API_BASE_URL"
            )
        ...
        async with httpx.AsyncClient(verify=self._settings.verify_ssl, timeout=timeout) as client:
            try:
                ...
                raise LottomaticapssRestError(f"REST request failed: {e}") from e
            ...
            if resp.status_code >= 400:
                ...
                raise LottomaticapssRestError(error_text, status_code=resp.status_code)
