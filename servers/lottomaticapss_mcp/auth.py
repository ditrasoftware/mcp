...
from .rest_client import LottomaticapssAuth
...
def auth_from_ctx(ctx: Context | None) -> LottomaticapssAuth:
    if ctx is None:
...
def apply_default_auth(auth: LottomaticapssAuth, *, default_api_key: str | None) -> LottomaticapssAuth:
...
def require_auth(auth: LottomaticapssAuth) -> None:
    if auth.access_token or auth.api_key:
        return
    raise ToolError(
