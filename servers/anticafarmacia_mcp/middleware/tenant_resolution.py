...
class TenantResolutionMiddleware(Middleware):
    ...
    async def on_list_tools(self, context, call_next):
        ...
        self._try_attach_tenant(context, tenant)
    ...
    def _try_attach_tenant(self, context: Context, tenant: TenantContext) -> None:
        """Attach tenant when middleware context allows mutation."""
        try:
            context.tenant = tenant
            return
        except (AttributeError, FrozenInstanceError):
            # Some FastMCP middleware contexts are immutable when assigned
            # through normal attribute writes. Try low-level setattr as fallback.
            try:
                object.__setattr__(context, "tenant", tenant)
                return
            except Exception:
                return
