...
class AnticaFarmaciaAuth:
    ...
    def merged(self, other: AnticaFarmaciaAuth) -> AnticaFarmaciaAuth:
...
class AnticaFarmaciaRestError(RuntimeError):
    def __init__(self, message: str, *, status_code: int | None = None):
        super().__init__(message)
        self.status_code = status_code


class AnticaFarmaciaRestClient:
    def __init__(self, settings: AnticaFarmaciaSettings):
    ...
    async def request(
        ...
        *,
        ...
        json: Any | None = None,
        auth: AnticaFarmaciaAuth | None = None,
        extra_headers: Mapping[str, str] | None = None,
        expect_json: bool = True,
    ) -> Any:
        if not self._settings.api_base_url:
            raise AnticaFarmaciaRestError(
                "AnticaFarmacia REST backend is not configured: set ANTICAFARMACIA_API_BASE_URL"
            )
        ...
        async with httpx.AsyncClient(verify=self._settings.verify_ssl, timeout=timeout) as client:
            try:
                ...
                raise AnticaFarmaciaRestError(f"REST request failed: {e}") from e
            ...
            if resp.status_code >= 400:
                ...
                raise AnticaFarmaciaRestError(error_text, status_code=resp.status_code)
