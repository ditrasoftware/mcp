# DitraSoftware Template MCP

Generic scaffolding and baseline structure for DitraSoftware MCP (Model Context Protocol) servers. This template provides:
...
export DITRASOFTWARE_API_BASE_URL="https://api.example.com"
export DITRASOFTWARE_API_TIMEOUT_SECONDS=30
export DITRASOFTWARE_VERIFY_SSL=true
export DITRASOFTWARE_DEFAULT_API_KEY=""  # Optional fallback

# Gateway configuration
export DITRASOFTWARE_GATEWAY_MODE=hybrid
export DITRASOFTWARE_GATEWAY_ROUTE_POLICY=local_preferred
export DITRASOFTWARE_GATEWAY_MOUNT_ON_STARTUP=true
export DITRASOFTWARE_GATEWAY_ALLOW_DIRECT_CALLS=true
export DITRASOFTWARE_GATEWAY_DIRECT_RESULT_STRATEGY=passthrough  # or normalized

# Optional: configure remote MCP backends
export DITRASOFTWARE_GATEWAY_REMOTES_JSON='[
  {
    "name": "toolbox-mssql",
...
export DITRASOFTWARE_GATEWAY_TOOL_ROUTE_OVERRIDES_JSON='{
...
```bash
python -m ditrasoftware_template_mcp
```
...
docker run -p 8001:8001 \
  -e DITRASOFTWARE_API_BASE_URL="https://api.example.com" \
...
- `docker-compose-mcp.yml`
  - Canonical compose file for this template folder.
  - MCP-focused local/dev stack using local `build: .`.
- `docker-compose.yml`
  - Not present in this folder by default.
...
If `DITRASOFTWARE_DEFAULT_API_KEY` is set, the server will use it as a fallback when no explicit credentials are provided.
...
```bash
export DITRASOFTWARE_DEFAULT_API_KEY="your-api-key"
...
```bash
# Don't set DITRASOFTWARE_DEFAULT_API_KEY
...
docker run --secret ditrasoftware_api_key \
  -e DITRASOFTWARE_DEFAULT_API_KEY_FILE=/run/secrets/ditrasoftware_api_key \
  ditrasoftware-mcp:latest
```
...
```bash
# Copy the template
cp -r ditrasoftware_template_mcp my_custom_mcp
cd my_custom_mcp
...
export DITRASOFTWARE_CACHE_TTL=300
export DITRASOFTWARE_CACHE_SCOPE=public

# Or set per-session if auth context matters:
export DITRASOFTWARE_CACHE_SCOPE=private
```
...
export DITRASOFTWARE_LIST_PAGE_SIZE=50
```
...
export DITRASOFTWARE_MASK_ERROR_DETAILS=true
```
...
DitraSoftware MCP Template - See LICENSE file
