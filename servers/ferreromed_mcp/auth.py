...
def auth_from_ctx(ctx: Context | None) -> FerreroMedAuth:
    if ctx is None:
    ...
    # Claude Desktop remote connectors sometimes can't send custom headers like X-Api-Key,
    # but can send "OAuth ID / password" which is transmitted as HTTP Basic auth.
...
def apply_default_auth(auth: FerreroMedAuth, *, default_api_key: str | None) -> FerreroMedAuth:
...
def require_auth(auth: FerreroMedAuth) -> None:
    if auth.access_token or auth.api_key:
        return
    raise ToolError(
