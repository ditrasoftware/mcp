...
def _candidate_env_names(remote: RemoteBackendSettings, key: str) -> list[str]:
    ...
    if remote_name:
        names.append(f"FERREROMED_GATEWAY_REMOTE_{remote_name}_{key}")
    if remote_namespace and remote_namespace != remote_name:
        names.append(f"FERREROMED_GATEWAY_REMOTE_{remote_namespace}_{key}")

    names.append(f"FERREROMED_GATEWAY_REMOTE_{key}")
