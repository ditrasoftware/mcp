...
def create_auth_provider() -> AuthProvider | None:
    ...
    mode = (_env("FERREROMED_MCP_AUTH_MODE") or "off").lower()
    if mode in {"off", "0", "false", "none", "disabled"}:
        return None

    base_url = _env("FERREROMED_MCP_BASE_URL")
    if not base_url:
        raise RuntimeError(
            "FERREROMED_MCP_AUTH_MODE is enabled but FERREROMED_MCP_BASE_URL is not set "
            "(example: https://ferreromed.ditra.io)"
        )

    resource_base_url = _env("FERREROMED_MCP_RESOURCE_BASE_URL")

    if mode in {"oidc_proxy", "oidc", "oidc-proxy"}:
        config_url = _env("FERREROMED_OIDC_CONFIG_URL")
        client_id = _env("FERREROMED_OIDC_CLIENT_ID")
        client_secret = _env("FERREROMED_OIDC_CLIENT_SECRET")
        jwt_signing_key = _env("FERREROMED_OIDC_JWT_SIGNING_KEY")

        if not config_url or not client_id:
            raise RuntimeError(
                "OIDC auth enabled but missing FERREROMED_OIDC_CONFIG_URL and/or FERREROMED_OIDC_CLIENT_ID"
            )
        if not client_secret and not jwt_signing_key:
            raise RuntimeError(
                "OIDC auth enabled but missing FERREROMED_OIDC_CLIENT_SECRET (or set FERREROMED_OIDC_JWT_SIGNING_KEY for public clients)"
            )

        required_scopes = _env_list("FERREROMED_OIDC_REQUIRED_SCOPES")
        allowed_redirect_uris = _env_list("FERREROMED_OIDC_ALLOWED_CLIENT_REDIRECT_URIS")
        verify_id_token = _env_bool("FERREROMED_OIDC_VERIFY_ID_TOKEN", False)
        token_endpoint_auth_method = _env("FERREROMED_OIDC_TOKEN_ENDPOINT_AUTH_METHOD")
    ...
    if mode in {"supabase"}:
        ...
        required_scopes = _env_list("FERREROMED_SUPABASE_REQUIRED_SCOPES")
        ...
        oauth_signing_secret = (
            _env("FERREROMED_OAUTH_SIGNING_SECRET")
            or _env("SUPABASE_JWT_SECRET")
            or _env("JWT_SECRET")
        )
        if not oauth_signing_secret:
            raise RuntimeError(
                "Supabase auth enabled but missing FERREROMED_OAUTH_SIGNING_SECRET (or SUPABASE_JWT_SECRET/JWT_SECRET)"
            )

        oauth_shared_secret = _env("FERREROMED_OAUTH_SHARED_SECRET")
        allowed_redirect_uris = _env_list("FERREROMED_OAUTH_ALLOWED_REDIRECT_URIS") or [
            "https://claude.ai/api/mcp/auth_callback"
        ]
        allowed_client_ids = _env_list("FERREROMED_OAUTH_ALLOWED_CLIENT_IDS")
        access_token_ttl_s = int(float(_env("FERREROMED_OAUTH_ACCESS_TOKEN_TTL_SECONDS") or "3600"))
        code_ttl_s = int(float(_env("FERREROMED_OAUTH_CODE_TTL_SECONDS") or "180"))
        ...
        class _StaticSupabaseAuthProvider(RemoteAuthProvider):
            def get_middleware(self) -> list:
                ...
                allow_api_key = _env_bool("FERREROMED_MCP_ALLOW_API_KEY_FALLBACK", True)

                token_verifier = self.token_verifier

                class _BearerOrApiKeyBackend(AuthenticationBackend):
                    async def authenticate(self, conn: HTTPConnection):
                        debug = _env_bool("FERREROMED_MCP_AUTH_DEBUG", False)
        ...
        return _StaticSupabaseAuthProvider(
            ...
            resource_base_url=resource_base_url,
            resource_name=_env("FERREROMED_MCP_RESOURCE_NAME"),
            resource_documentation=(AnyHttpUrl(_env("FERREROMED_MCP_RESOURCE_DOCUMENTATION")) if _env("FERREROMED_MCP_RESOURCE_DOCUMENTATION") else None),  # type: ignore[call-arg]
        )

    raise RuntimeError(
        f"Unknown FERREROMED_MCP_AUTH_MODE: {mode!r} (expected off|oidc_proxy|supabase)"
    )
